...
class QuestionRepository
{
    ...
    {
        try {
            return Question::where('TutorID', Auth::guard('tutor')->user()->TutorID)
    ...
    public function edit($question_id)
    {
        try {
            return Question::with(['hint' => function ($query) {
                        ...
                        }])->select('QuestionID', 'QuestionText', 'QuestionTypeID', 'Marks', 'DifficultyLevelID', 'TutorID', 'IsActive', 'QuestionImage')
                        ->find($question_id);
        } catch (\Exception $e) {
            echo $e->getMessage();
            Log::channel('loginfo')
...
/*            print_r($data->toArray());
            ...
            // $bssctid = $query->select('BoardStandardSubjectChapterTopicID')->get()->pluck('BoardStandardSubjectChapterTopicID');
            ...
            //die;
            // $questions = Question::with(['bssctQuestion' => function($query) use ($bssctid) {
            //                     $query->whereIn('BSSCTopicID', $bssctid);
            //                 }])->whereHas('bssctQuestion', function($query) use ($bssctid) {
            //                     $query->whereIn('BSSCTopicID', $bssctid);
            //                 });
            ...
            $questions = $questions->where('QuestionTypeID', $question_type_id)->get();
            return $questions;
            /*echo "()()()";
    ...
    {   
        try {
            return TestSectionQuestionType::with(['section' => function($query) {
                                ...
                                /*->select('QuestionID', 'NumberofQuestion')*/
                                ->first();
        } catch (\Exception $e) {
            Log::channel('loginfo')
    ...
    {
        //return Laratables::recordsOf(Question::class);
        try {
            //echo "<pre>";
            $data = TestPackageTest::with(['testPackage' => function($query) {
                                    $query->select('TestPackageID', 'IsCompetetive', 'NumberOfTest', 'QuestionFrom', 'IsAutoTestCreation', 'BoardID', 'StandardID', 'SubjectID', 'CourseID', 'TutorID', 'IsActive');
