                    ...
                    <ul class="dropdown-menu user-auth-dropdown p-10" data-dropdown-in="flipInX" data-dropdown-out="flipOutX">
                        <li class="text-center">
                            @if (Auth::guard('admin')->check())
                                ...
                                <span><b>{{Auth::guard('admin')->user()->UserEmail}}</b></span>
                            @endif
                            @if (Auth::guard('tutor')->check())
                                <span><b>{{Auth::guard('tutor')->user()->TutorName}}</b></span><br>
                                <span><b>{{Auth::guard('tutor')->user()->TutorEmail}}</b></span>
                            @endif
                        </li>
                        <li class="divider"></li>
                        <li>
                            @if (Auth::guard('admin')->check())
                                <a href="{{route('admin_profile')}}"><span>Profile</span></a>
                            @endif
                            @if (Auth::guard('tutor')->check())
                                <a href="{{route('tutor_profile')}}"><span>Profile</span></a>
                            @endif
                        </li>
                        <li class="divider"></li>
                        <li>
