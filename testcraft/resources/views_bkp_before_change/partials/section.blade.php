    ...
    <div id="" class="panel-heading panel-head">
        ...
        <div>
            ...
            @foreach($questionType->testQuestion as $question)
                <tr>
                    <td style="width: 70%"> {!! $question->question->QuestionText !!} </td>
                    ...
                    <td style="width: 10%"> 
                        @if($section->test->StatusID != 9)
                        <a href="#" data-toggle="modal" data-tqID="{{$question->TestQuestionManualID}}" data-target="#deleteQuestion" class="btn-link pull-right" data-testID="{{$section->TestPackageTestID}}"><i class="fa fa-close"></i></a> 
                        @endif
                    </td>
                </tr>
            @endforeach
